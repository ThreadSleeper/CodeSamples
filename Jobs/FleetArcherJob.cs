...
[BurstCompile]
public struct FleetArcherJob : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        if (readyToAttack)
        {
            ...
            targetPosition += new float3(Randomizer.Float(-20, 20, ref seed), 0, Randomizer.Float(-20, 20, ref seed)); // todo maybe use randomizer if units size adjusted

            float3 relativeVector = targetPosition - t.Position;
            float distance = math.length(relativeVector.xz);
            float3 arrowStartingPosition = t.Position + new float3(0f, 0f, 0f);
            ...
            angle = Randomizer.Float(angle - variant, angle + variant, ref seed); // todo maybe use randomizer if units size adjusted
